...
### 1) 旧検索モード（legacy=1）
一覧画面で `legacy=1` を付けると、**ユーザー入力を元に生SQL文字列を組み立てる**実装が動きます。

- 例：`/?q=abc&legacy=1`
- **明らかな症状:** 検索文字にシングルクォート `'` が入ると 500 になったりします（教材用）
...
## 授業でのおすすめの直す順番（超要約）
1. 旧検索（legacy）の“壊れやすさ”を直す（まずは `'` で落ちるのを再現→修正）
2. ページネーション追加
